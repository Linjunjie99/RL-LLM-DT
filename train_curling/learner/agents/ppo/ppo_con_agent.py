...
@agent_registry.register('ppo_con')
class PPOCAgent(Agent):
    def __init__(self, model_cls, observation_space, action_space, config=None,
        ...
        super(PPOCAgent, self).__init__(model_cls, observation_space, action_space, config, *args, **kwargs)

    def build(self) -> None:
        self.model = self.model_instances[-1]

        self.entropy = tf.reduce_mean(self.model.neglogp_a_ph)
        ...
        ratio = tf.exp(self.old_neglogp_ph - self.model.neglogp_a_ph)
        ...
        # Stat
        self.kl = tf.reduce_mean(self.model.neglogp_a_ph - self.old_neglogp_ph)
        ...
        trainer = tf.train.RMSPropOptimizer(learning_rate=self.lr_ph, epsilon=1e-5)
    ...
    def learn(self, training_data, *args, **kwargs):
        ...
        for k, v in stats.items():
            if k!='mu':
                re_dic[k] = np.array(v).mean()
            else:
                re_dic['mu0'] = np.array(v)[:,:,0].mean()
                re_dic['mu1'] = np.array(v)[:,:,1].mean()

        # return {k: np.array(v).mean(axis = 0) for k, v in stats.items()}
        return re_dic
    ...
    def train(self, obs, returns, actions, values, neglogps):
        ...
        _, neg_logp_a, mu, pg_loss, vf_loss, entropy, clip_rate, kl = self.sess.run(
            [self.train_op, self.model.neglogp_a_ph, self.model.mu, self.pg_loss, self.vf_loss, self.entropy, self.clip_rate, self.kl], td_map)
        return {
            'neg_logp_a': neg_logp_a,
            'mu': mu,
            'pg_loss': pg_loss,
            'vf_loss': vf_loss,
            'kl': kl
