...
class PPOAgent(Agent):
    def __init__(self, model_cls, observation_space, action_space, config=None,
                 gamma=0.99, lam=0.95, lr=5e-5, clip_range=0.1, ent_coef=0.01, vf_coef=0.5, max_grad_norm=0.5,
    ...
    def build(self) -> None:
        ...
        clipped = tf.logical_or(tf.logical_and(ratio > (1 + self.clip_range), self.advantage_ph > 0), tf.logical_and(ratio < (1 - self.clip_range), self.advantage_ph < 0))
